...
const createAdminTask = async (req, res) => {
  ...
  const imageUrl = req.file ? `/uploads/${req.file.filename || 'uploaded_image'}` : null;
  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        const aiResponse = await axios.post('http://127.0.0.1:8000/extract-boundary', formData, {
  ...
  try {
    ...
    if (checkResult.rows[0].status === 'Approved') {
      return res.status(403).json({ 
        error: "Forbidden: Approved parcel boundaries are locked and cannot be modified." 
      });
    }
...
const auditDecision = async (req, res) => {
  ...
  if (!parcel_id || !['Approved', 'Rejected'].includes(decision)) {
    return res.status(400).json({
      error: "Valid parcel_id and decision ('Approved' or 'Rejected') are required."
    });
  }

  try {
    // Decision Reject hone par status automatically 'Pending Survey' hoga taaki Surveyor correction kar sake
    const targetStatus = decision === 'Approved' ? 'Approved' : 'Pending Survey';
    ...
    const result = await pool.query(query, [targetStatus, auditor_comments || '', parcel_id]);
    ...
    return res.status(500).json({ error: "Internal server audit processing error." });
  }
};
...
const autoTraceParcels = async (req, res) => {
  ...
  try {
    ...
    const aiResponse = await axios.post('http://127.0.0.1:8000/extract-all-parcels', formData, {
      headers: { ...formData.getHeaders() },
      timeout: 180000
    });
    ...
    if (features.length === 0) {
      return res.status(400).json({ error: "No traceable boundaries found in image." });
    }
    ...
    console.error("Auto trace backend error:", err.response?.data || err.message);
